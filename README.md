...
File monitoring currently uses Windows `FileSystemWatcher` and records metadata within the current user's accessible scope. Installer reports associate those events with an installer session by time window; this is explicitly labeled as best-effort and is not presented as exact process attribution. TraceGuard safely probes whether the current user can query each NTFS USN Journal, but journal event reading is not yet enabled. It also checks whether the current token already belongs to Windows' `Performance Log Users` group before presenting ETW session control as eligible. Neither probe starts a trace session, creates or modifies a journal, changes group membership, or requests elevation. Disk activity aggregates I/O counters from processes readable by the current user, so elevated or protected-process activity can remain unaccounted for. The UI labels these boundaries instead of inventing values.
