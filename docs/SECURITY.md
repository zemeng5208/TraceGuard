...
Windows restricts general ETW session control to administrators, the built-in `Performance Log Users` group, and specific service accounts; NT Kernel Logger control is stricter. TraceGuard only inspects the current token for existing `Performance Log Users` membership. If already eligible, it may start a uniquely named normal real-time session for the process and file manifest providers, using bounded metadata keywords and an in-memory path/PID correlation cache. It never adds the user to that group, requests elevation, starts the NT Kernel Logger, enables stack capture, records file contents, or persists raw ETW payloads. Provider/session failures degrade to the existing collectors.
