  ...
  return (
    <section className="data-page panel">
      ...
      <div className="table-wrap">
        <table><thead><tr><th>{settings.locale === 'zh-CN' ? '进程' : 'Process'}</th><th>PID</th><th>CPU</th><th>RAM</th><th>{settings.locale === 'zh-CN' ? '路径' : 'Path'}</th><th>{settings.locale === 'zh-CN' ? '权限' : 'Permission'}</th><th /></tr></thead>
          <tbody>{filtered.map((row) => <tr key={row.pid}><td><strong>{row.name}</strong><small>{row.publisher ?? 'Unknown publisher'}</small></td><td>{row.pid}</td><td>{row.cpuPercent.toFixed(1)}%</td><td>{Math.round(row.memoryBytes / 1024 / 1024)} MB</td><td className="path-cell">{row.executable ?? '—'}</td><td><PermissionBadge permission={row.permission} settings={settings} /></td><td><button className="icon-action" type="button" disabled={row.permission !== 'controllable'} onClick={() => void api.stopProcess(row.pid)} title={settings.locale === 'zh-CN' ? '停止一次' : 'Stop once'}><StopCircle size={16} /></button></td></tr>)}</tbody>
