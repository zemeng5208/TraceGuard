...
function StandardContent({ section, settings, onChange, overview }: SettingsContentProps & { section: SectionId }) {
  ...
  const statusLabel = (status?: MonitorModuleStatus) => {
    ...
    const labels = isZh
      ? { active: '运行中', available: '可用', reduced: '已降频', paused: '已暂停', disabled: '已关闭', unavailable: '不可用' }
      : { active: 'Active', available: 'Available', reduced: 'Reduced', paused: 'Paused', disabled: 'Disabled', unavailable: 'Unavailable' };
  ...
  else if (section === 'advanced') { const fileStatus = statusFor('file'); const usnStatus = statusFor('usn'); const etwStatus = statusFor('etw'); body = <><div className="advanced-warning"><Bot size={21} /><span>{isZh ? '高级设置可能影响 TraceGuard 的行为，建议普通用户保持默认值。' : 'Advanced settings can change TraceGuard behavior. Default values are recommended.'}</span></div><SettingCard title={isZh ? '采集能力状态' : 'Collection capability status'}><SettingRow title="FileSystemWatcher" description={fileStatus ? (isZh ? fileStatus.messageZh : fileStatus.message) : (isZh ? '正在读取真实运行状态。' : 'Reading actual runtime status.')}><span className={`module-status status-${fileStatus?.state ?? 'checking'}`}>{statusLabel(fileStatus)}</span></SettingRow><SettingRow title="USN Journal" description={usnStatus ? (isZh ? usnStatus.messageZh : usnStatus.message) : (isZh ? '正在以当前用户权限探测，不创建或修改日志。' : 'Probing with the current user token without creating or modifying a journal.')}><span className={`module-status status-${usnStatus?.state ?? 'checking'}`}>{statusLabel(usnStatus)}</span></SettingRow><SettingRow title="ETW Attribution" description={etwStatus ? (isZh ? etwStatus.messageZh : etwStatus.message) : (isZh ? '正在检查当前用户令牌资格，不启动跟踪会话。' : 'Checking current-token eligibility without starting a trace session.')}><span className={`module-status status-${etwStatus?.state ?? 'checking'}`}>{statusLabel(etwStatus)}</span></SettingRow></SettingCard></>; }
  else body = <SettingCard title={`TraceGuard ${TRACEGUARD_VERSION}`}><div className="about-mark"><span className="brand-shield">TG</span><div><strong>See what changed.</strong><span>Understand why. Control what you own.</span><small>Windows 10 / 11 · Zero-Privilege · Local First</small></div></div></SettingCard>;
