...
export function StartupPage({ rows, settings }: { rows: StartupRow[]; settings: AppSettings }) {
  ...
  return (
    <section className="data-page panel">
      ...
      <div className="table-wrap"><table><thead><tr><th>{isChinese(settings.locale) ? '启动项' : 'Startup item'}</th><th>{isChinese(settings.locale) ? '来源' : 'Source'}</th><th>{isChinese(settings.locale) ? '命令' : 'Command'}</th><th>{isChinese(settings.locale) ? '状态' : 'Status'}</th><th>{isChinese(settings.locale) ? '权限' : 'Permission'}</th></tr></thead>
