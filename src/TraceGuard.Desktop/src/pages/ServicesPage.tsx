...
export function ServicesPage({ rows, settings }: { rows: ServiceRow[]; settings: AppSettings }) {
  ...
  return (
    <section className="data-page panel">
      ...
      <div className="table-wrap"><table><thead><tr><th>{isChinese(settings.locale) ? '服务' : 'Service'}</th><th>{isChinese(settings.locale) ? '状态' : 'Status'}</th><th>{isChinese(settings.locale) ? '启动类型' : 'Startup type'}</th><th>{isChinese(settings.locale) ? '分类' : 'Category'}</th><th>{isChinese(settings.locale) ? '权限' : 'Permission'}</th><th /></tr></thead>
