...
export function PlannedPage({ page, settings }: { page: Exclude<PageId, 'dashboard' | 'terminal' | 'processes' | 'services' | 'startup' | 'settings'>; settings: AppSettings }) {
  ...
  return <section className="planned-page panel"><span className="planned-icon"><Construction size={34} /></span><h2>{text(key, settings.locale)}</h2><p>{text('phasePlanned', settings.locale)}</p><small>{secondaryText(key, settings.locale)}</small><button type="button" disabled><Settings2 size={16} />{settings.locale === 'zh-CN' ? '此页面不会显示虚假控制项' : 'No placeholder controls are exposed'}</button></section>;
}
